...
jid4=$(get_jobid bsub -w "ended($jid3)" < $SCRIPT_PATH/pause.bsub)
echo Submitted pause job for $DATA_BASE_NAME with ID: $jid4

jid5=$(get_jobid bsub -w "ended($jid4)" < $SCRIPT_PATH/bigwig.bsub)
echo Submitted bigwig job for $DATA_BASE_NAME with ID: $jid5
